...
ToneAlarm::ToneAlarm()
{
    ...
    if(period_fd==-1){
        printf("Erros openning /sys/devices/ocp.3/pwm_test_P8_36.12/period\n");
        exit(-1);
    }
    duty_fd = open("/sys/devices/ocp.3/pwm_test_P8_36.12/duty",O_WRONLY);
    if(duty_fd==-1){
        printf("Erros openning /sys/devices/ocp.3/pwm_test_P8_36.12/duty");
        exit(-1);
    }

    run_fd = open("/sys/devices/ocp.3/pwm_test_P8_36.12/run",O_WRONLY);
    if(run_fd==-1){
        printf("Erros openning /sys/devices/ocp.3/pwm_test_P8_36.12/run_fd");
        exit(-1);
    }
...
bool ToneAlarm::init()
{
    ...
    if((period_fd == -1) || (duty_fd == -1) || (run_fd == -1)){
        printf("ToneAlarm: Error!! please check if PWM overlays are loaded correctly");
        return false;
    }
    return true;
}
...
uint32_t ToneAlarm::millis()
{
    return millis64() & 0xFFFFFFFF;
}

uint64_t ToneAlarm::millis64()
{
    struct timespec ts;
